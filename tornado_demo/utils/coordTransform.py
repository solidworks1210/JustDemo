...
# 以下新增：2017-09-05
def wgs84_to_bd09_source_list(pt_list):
    """
    wgs84 to bd09
    :param pt_list:[(lng, lat),...]
    :return: bd09 point list , if only on point return a point tuple
...
def wgs84_to_gcj02_source_list(pt_list):
    """
    wgs84 to gcj02
    :param pt_list:[(lng, lat),...]
    :return: gcj02 point list , if only on point return a point tuple
    """
    if not isinstance(pt_list, list):
        raise TypeError('need a list.')
    result_list = []
    if len(pt_list) >= 1:
        for pt in pt_list:
            ...
            lng_02, lat_02 = wgs84_to_gcj02(lng, lat)
            # 使用高德地图
            result_list.append((round(lng_02, 6), round(lat_02, 6)))
    if len(result_list) == 1:
        return result_list[0]
    else:
        return result_list
...
def wgs84_to_map_coordinate(pt_list, coordinate_type=None):
    """
    ...
    :return: [(lng, lat),...]
    """
    if coordinate_type == 'bd09':
        return wgs84_to_bd09_source_list(pt_list)
    elif coordinate_type == 'gcj02':
        return wgs84_to_gcj02_source_list(pt_list)
    else:
        return wgs84_to_gcj02_source_list(pt_list)


def bd09_to_wgs84_source_list(pt_list):
    """
    bd09 to wgs84
    :param pt_list:[(lng, lat),...]
    :return: a point tuple list. if only on point return a point tuple
...
def gcj02_to_wgs84_source_list(pt_list):
    """
    gcj02 to wgs84
    :param pt_list:[(lng, lat),...]
    :return: a point tuple list. if only on point return a point tuple
    ...
    if len(pt_list) >= 1:
        for pt in pt_list:
            lng, lat = pt[0], pt[1]
            lng_84, lat_84 = gcj02_to_wgs84(lng, lat)
            result_list.append((round(lng_84, 6), round(lat_84, 6)))
    if len(result_list) == 1:
        return result_list[0]
    else:
...
def map_coordinate_to_wgs84(pt_list, coordinate_type=None):
    """
    地图坐标转换为wgs84
    :param pt_list:
    :param coordinate_type: None 默认， 'bd09' 百度，'gcj02' 火星
    ...
    """
    if coordinate_type == 'bd09':
        return bd09_to_wgs84_source_list(pt_list)
    elif coordinate_type == 'gcj02':
        return gcj02_to_wgs84_source_list(pt_list)
    else:
        return gcj02_to_wgs84_source_list(pt_list)
